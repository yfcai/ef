...
\end{enumerate}
We demonstrate a technique to achieve all of the above and more:
...
\end{enumerate}
The technique puts a slight restriction on what a type can be.
Say a program $f$ approximates the program $g$ if $f$ terminates
on fewer inputs than $g$ but otherwise behaves just like $g$.
Every type $\tau$ must satisfy the following conditions:
\begin{itemize}
...
performance. In particular, the type checker may not terminate.
Writing a nonterminating type checker is as easy as writing a
nonterminating program in Java.
