...
recursive types are well-defined.
...
\item two distinct base values in $B$ are incomparable by $\Sub$,
...
closure under directed limit are necessary to establish
well-definedness of recursive types. Moreover, closure under
directed limit is a natural strengthening of the
\emph{admissibility} criterion for contracts in HALO (Haskell to
logic through denotational semantics); in fact, all contracts
described in HALO can be strengthened to types by adding a
downward-closure condition without disturbing their theoretic
development.
...
call its argument $g$ a finite number of times before returning a
result. Even if $g$ is more complex than a finite number of
argument-result pairs, the result $f(g)$ does not reflect it.
...
\item[R4.] If $f\in V \R V$ does not have rank $n$ for all
$n\in\mathbb N$, then the rank of $f$ is $\infty$.
...
\textsc{F-0}: Since $t$ and $\sigma$ are closed, they denote a
...
term, so that F-rules can deal with it in comfort. Ascription
suffices.
