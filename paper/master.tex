...
\Cps(\Gamma \vdash s~t : \tau) & =
  ...
  \hspace{9em minus 1em} \mbox{where}
  \\
...
$\Undo$, $\Redo$ from now on.

Case $\tau = \alpha$ or $\tau = \bot$: Set aside the
ridiculosity. Brouwer's double negation elimination is $\Undo$
and double negation introduction is $\Redo$.
\end{proof}
