...
optional type arguments, and subtyping. It is interesting to
...
We can start using F and CT1 together simply by mixing their
typing rules. However, F consumes produces F-judgements, while
CT1 consumes and produces S- and CT-judgements. To use rules from
both systems in the typing derivation of a single term, we need
to make F- and CT-judgements talk to each other by establishing
some entailment relations.

Going from F-judgements to CT-judgements is easy, because the
former are a stronger statements than the latter.
...
do we have to resolve accumulated constraints by e.~g.\
\textsc{CT-S}, we must also eliminate the union, intersection and
interval-bounded universal type expression constructors from the
type of a term, so that F-rules can handle it comfortably.
Ascription suffices.
...
\Gamma&\vdash t:\rho\Given C
\\
\beta_1\in I_1,\ldots,\beta_n\in I_n&\vdash C,\rho\le\sigma
\end{array}
\andalso
