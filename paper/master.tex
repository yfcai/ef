...
we have
\[
\Sem{\Erase(t)}~\env~\in~\Sem\sigma~\Env.
\]
\end{quotation}~
...
defined on $\Delta$. Write $t_0=\Erase(t)$. By the antecedent,
$t_0$ denotes a value in $\Sem{\sigma_2}~\Env$ under every
environment $\env$ that is compatible with $\Gamma$ and maps $x$
to a member of $\Sem{\sigma_1}~\Env$. It follows that the
denotation of $\Abs xt_0$ under every environment compatible with
$\Gamma$ is a function whose image of $\Sem{\sigma_1}~\Env$ is a
subset of $\Sem{\sigma_2}~\Env$.
...
compatible with $\Gamma$ under $\Env$. Let
\[
v=\Sem{\Erase(t)}~\env.
\]
...
\textsc{T-Tapp}: Something about
\[
\Sem{\sigma_0[\alpha\mapsto\sigma_1]}~\Env
=
\Sem{\sigma_0}~(\Update\Env\alpha\mapsto\Sem{\sigma_1}~\Env)
\]
