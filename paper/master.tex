...
of data type generic programming.
...
cannot detect that extra complexity and cannot reflect it in the
result $f(g)$.
...
only if there exists a set $S$ of pairs of values of rank at most
...
element, then $S=\{\bot\}$ and $\sup S=\bot$. If $S$ has a rank-1
element $v$, then $v$ is maximal in $V$ and has to be an upper
...
\penalty-1000 % really like to page break before a paragraph
...
from $H$ takes two nonexpansive type functions to a contractive
one. Such is the case in L3. We will prove neither the preceding
remarks nor the nonexpansion of union and intersection, because I
...
will be discussed in the next theorem.
...
with $P$. Let
\begin{align*}
S &= \{x' \Or x'\Sub x\text{ and }(x', y')\in P\text{ for some }y'\},
\end{align*}
$S$ has a supremum because $x$ is an upper bound of $S$. Since
$f$ is least,
\[
\sup S=f(x)=y.
\]
By lemma~\ref{sup-rank}, the rank of $y$ is at most $n-1$. Thus
\[
d(A\RT C,B\RT C)=\frac1{2^n}=\frac12\cdot\frac1{2^{n-1}}
\le\frac12\cdot\frac1{2^{\Rank(y)}}\le\frac12\,d(B,C).
\]
\end{proof}
...
are maximal in $V$, no value can be both variants at the same
time. In general,
\[
...
such as System~F, it is desirable if we do not exclude universal
types from the body of a recursive type. We may see types like
\begin{align*}
...
bound type variable at a time, which requires a more powerful
framework than KISP.
...
expertise to verify it, it seems reasonable to assume that the
...
model, but only use fragments appropriate to the situation at
hand.
