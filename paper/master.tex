...
Cauchy sequence of types. We will show that the type $T$ below is
well-defined, and that $T_i$ is arbitrarily close to $T$ for all
$i$ large enough.
\begin{align*}
T &
= \Closure(\limsup_{n\R\infty} T_n)
= \Closure(\liminf_{n\R\infty} T_n)
\\
\limsup_{n\rightarrow\infty} T_n&
= {\textstyle\bigcap_{n=1}^\infty\bigcup_{i=n}^\infty} T_i\\
\liminf_{n\rightarrow\infty} T_n&
={\textstyle \bigcup_{n=1}^\infty\bigcap_{i=n}^\infty} T_i
\end{align*}

$T$ is well-defined only if
\[
\Closure(\liminf T_n)=\Closure(\limsup T_n).
\]
Knowing the standard result $\liminf T_n\subseteq\limsup T_n$, we
need only show
\[
\Closure(\limsup T_n)\subseteq\Closure(\liminf T_n),
\]
which follows, by algebraicity and lemma~\ref{closure}, if every
compact element of $\limsup T_n$ is a member of $\liminf T_n$.

\def\Cauchy{\texttt{cauchy}}
\def\Hasv{{\texttt{has{\textunderscore}v}}}

Choose an arbitrary compact element $v\in \limsup T_n$. Let
$\epsilon=2^{-\Rank(v)}>0$. There exists a natural number
...
\[
v\in
{\textstyle\bigcap_{i=\Hasv}^\infty}\, T_i\subseteq
{\textstyle\bigcup_{n=1}^\infty\bigcap_{i = n}^\infty} T_i.
=\liminf T_n.
\]

It remains to verify that $T$ is a type and is arbitrarily closed
to $T_i$ for all $i$ large enough.
...
+ 1$. By lemma~\ref{closure}, $v\in\liminf T_j - T_{\ge i}$,
